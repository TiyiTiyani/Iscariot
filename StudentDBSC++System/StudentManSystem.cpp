...
void get_file()
{
	...
	int i=0;
	fstream f;
	f.open("PrimarySchool.txt",ios::in);
	if(f)
	{
...
int search_grade(char grade[],vector<int> &vj)
{
	int i,ta=0;
	for(i=0;i<v.size();i++)
	if(strcmp(v[i].grade,grade)==0)
	{
		ta=1;
		vj.push_back(i);
	}
